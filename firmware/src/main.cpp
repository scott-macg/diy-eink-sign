...
void setup() {
    ...
    // Configure Boot / Force-Refresh Switch pin
    pinMode(BOOT_BTN_PIN, INPUT_PULLUP);

    // Inspect Wakeup Reason
    esp_sleep_wakeup_cause_t wakeup_reason = esp_sleep_get_wakeup_cause();
    bool isForceRefresh = false;

    if (wakeup_reason == ESP_SLEEP_WAKEUP_GPIO || wakeup_reason == ESP_SLEEP_WAKEUP_EXT0) {
        Serial.println("[WAKE] Woken up manually via Boot / Force-Refresh Microswitch!");
        playSoundBootWake();
        isForceRefresh = true;
    } else {
        Serial.println("[WAKE] Woken up by timer or initial cold power-on.");
    }

    // Open Preferences storage
    preferences.begin("epaper", false);
    String storedETag = isForceRefresh ? "" : preferences.getString("etag", "");
    ...
    if (WiFi.status() != WL_CONNECTED) {
        ...
        // Prepare HTTP / HTTPS GET Request
        HTTPClient http;
        WiFiClientSecure secureClient;
        String url = String(SERVER_URL);

        if (url.startsWith("https://")) {
            secureClient.setInsecure(); // Skip certificate verification for cloud PaaS
